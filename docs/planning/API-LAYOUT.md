...
## 発生箇所での例外応答（2026年9月15日追補）

シーケンスは例外の発生箇所でcatchの実際の結果を展開する。例外の既知のcodeなどと判定関数のASTから再送出か継続かを解決し、実装コード自体は実行しない。4xx・5xxを返す経路は、その場所から利用者への矢印と終了枠を描く。catchの行番号への案内、全業務例外の再送出一覧、末尾の共通例外経路は作らない。

入力形式・認証の失敗は業務処理の前、DB接続・SQL実行・commit・実体アクセス・モデル呼出しの失敗はそれぞれの呼出し位置に描く。内部で捕捉する型を区別し、根拠の欠落は候補の除外、モデル失敗は失敗状態での再認可と保存、索引失敗は配送結果の更新へつなげる。continueは当該候補の終了、returnは当該関数の終了、APIのreturnは利用者へのHTTP応答として区別する。保存済み回答は、元のtransactionのrollbackと再取得・再認可を発生位置に展開して200を返す。例外型・属性・再送出元を解決できない場合に架空のHTTP応答を補わない。
