...
| kotorelay.operations.images.get_ocr.functions.ocr | 17 | If | version_id |
| kotorelay.operations.images.get_ocr.functions.ocr | 28 | Return | result.model_copy(update={'confirmed': run.confirmed, 'regions': [r.model_copy(update={'region_id': r.region_id or stable_id(run.id + ':' + str(i))}) for i, r in enumerate(result.regions)]}) |
...
| kotorelay.operations.images.get_ocr.router.get_ocr | 24 | Return | build_response(f.ocr(ctx, str(run_id), str(version_id) if version_id else None)) |
| kotorelay.operations.images.shared.functions.authorize_asset | 16 | If | version_id is None |
| kotorelay.operations.images.shared.functions.authorize_asset | 31 | Return | asset |
