...
対象: `backend/src/kotorelay/operations/images/upload_image/functions.py:94`。式: `len(assets) < ctx.settings.max_document_images`
...
対象: `backend/src/kotorelay/operations/images/upload_image/functions.py:24`。式: `0 < len(data) <= max_bytes`
...
対象: `backend/src/kotorelay/operations/images/upload_image/functions.py:27`。式: `source.format in {'PNG', 'JPEG'} and source.width * source.height <= max_pixels and (max(source.width, source.height) <= 8000)`
...
対象: `backend/src/kotorelay/operations/images/upload_image/functions.py:40`。式: `len(value) <= max_bytes`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F10-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F10-false | 不成立 | 'invalid_image' / 422 |


### F11 条件分岐

対象: `backend/src/kotorelay/operations/images/upload_image/functions.py:62`。式: `text`
