...
対象: `backend/src/kotorelay/operations/images/functions.py:20`。式: `0 < len(data) <= max_bytes`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F07-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F07-false | 不成立 | 'invalid_image' / 422 |


### F08 条件分岐

対象: `backend/src/kotorelay/operations/images/functions.py:23`。式: `source.format in {'PNG', 'JPEG'} and source.width * source.height <= max_pixels and (max(source.width, source.height) <= 8000)`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F08-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F08-false | 不成立 | 'invalid_image' / 422 |


### F09 条件分岐

対象: `backend/src/kotorelay/operations/images/functions.py:36`。式: `len(value) <= max_bytes`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F09-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F09-false | 不成立 | 'invalid_image' / 422 |


### F10 条件分岐

対象: `backend/src/kotorelay/operations/images/functions.py:58`。式: `text`
...
対象: `backend/src/kotorelay/operations/images/functions.py:77`。式: `len(assets) < ctx.settings.max_document_images`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F11-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F11-false | 不成立 | 'limit' / 422 |
