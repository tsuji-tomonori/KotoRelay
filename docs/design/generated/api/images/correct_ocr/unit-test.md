...
対象: `backend/src/kotorelay/operations/images/functions.py:116`。式: `bool(assets)`
...
対象: `backend/src/kotorelay/operations/images/functions.py:120`。式: `len(ids) == len(set(ids))`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F08-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F08-false | 不成立 | 'invalid_region' / 422 |


### F09 条件分岐

対象: `backend/src/kotorelay/operations/images/functions.py:122`。式: `region.x + region.width <= 1.000001 and region.y + region.height <= 1.000001`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F09-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F09-false | 不成立 | 'invalid_region' / 422 |
