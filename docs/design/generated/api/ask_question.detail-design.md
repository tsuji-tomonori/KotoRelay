...
| backend/src/kotorelay/operations/chat/functions.py:206 | ctx.member(prepared.department_id) | 'forbidden' | 403 |
...
| backend/src/kotorelay/operations/chat/functions.py:175 | resumed is None | then / else の実装分岐 | 制御フロー参照 |
...
| backend/src/kotorelay/operations/chat/functions.py:125 | chunk.document_id not in docs or chunk.version_id != docs[chunk.document_id].latest_version_id or (not chunk.ready) | then / else の実装分岐 | 制御フロー参照 |
| backend/src/kotorelay/operations/chat/functions.py:131 | vector_keys is not None and chunk.id not in vector_keys | then / else の実装分岐 | 制御フロー参照 |
| backend/src/kotorelay/operations/chat/functions.py:142 | score > 0 and (vector_keys is not None or score >= max(1, len(terms(data.question)) * 0.3)) | then / else の実装分岐 | 制御フロー参照 |
| backend/src/kotorelay/operations/chat/functions.py:162 | not validate_citation(ctx, citation) | then / else の実装分岐 | 制御フロー参照 |
| backend/src/kotorelay/operations/chat/functions.py:166 | len(images) + len(related) > ctx.settings.max_model_images | then / else の実装分岐 | 制御フロー参照 |
| backend/src/kotorelay/operations/chat/functions.py:173 | len(citations) >= 5 | then / else の実装分岐 | 制御フロー参照 |
...
| backend/src/kotorelay/operations/chat/functions.py:260 | present(ctx, row) |
| backend/src/kotorelay/operations/chat/functions.py:192 | Prepared(answer_id=answer_id, conversation_id=conversation_id, question=data.question, department_id=data.department_id, citations=citations, texts=texts, images=images) |
