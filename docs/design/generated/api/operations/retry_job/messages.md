...
| --- | --- |
| operation | retry_job |
| endpoint | POST /api/operations/jobs/{job_id} |
...
| KR_INDEX_FAILED | ERROR | 索引または削除ジョブの実行が失敗しました。 |
...
| 呼出し位置 | backend/src/kotorelay/operations/indexing/shared/workflow.py:110, backend/src/kotorelay/operations/indexing/shared/workflow.py:115 |
...
| Problem | 422 | limit | 利用上限に達しました。 | KR_HTTP_REJECTED |
| Problem | 429 | limit | 利用上限に達しました。 | KR_HTTP_REJECTED |
| Problem | 503 | integrity | 保存内容の整合性を確認できません。 | KR_HTTP_FAILED |
