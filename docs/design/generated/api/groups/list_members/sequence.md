...
| kotorelay.operations.groups.list_members.functions.members | 13 | Return | [{'membership': m, 'display_name': users[m.user_id].display_name} for m in q.memberships_list(ctx.db, ctx.org) if m.department_id == department_id] |
...
| kotorelay.operations.groups.list_members.router.list_members | 23 | Return | build_response(f.members(ctx, str(department_id))) |
