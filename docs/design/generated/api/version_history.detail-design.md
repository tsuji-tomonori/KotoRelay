...
| backend/src/kotorelay/generated/queries.py:700 | db.query('operations/reviews/sql/submissions_list.sql', {'organization_id': organization_id}, SubmissionsRow) |
...
| backend/src/kotorelay/operations/documents/functions.py:225 | [{'version': v, 'submission': submissions.get(v.id)} for v in sorted(q.versions_list(ctx.db, ctx.org), key=lambda v: v.number, reverse=True) if v.document_id == doc.id] |
| backend/src/kotorelay/operations/documents/router.py:60 | f.history(ctx, str(document_id)) |
