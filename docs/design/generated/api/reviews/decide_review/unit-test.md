...
対象: `backend/src/kotorelay/context.py:82`。式: `bool(rows)`
...
対象: `backend/src/kotorelay/context.py:89`。式: `allowed`
...
| F07-false | 不成立 | 'idempotency_conflict' / 409 |


### F08 条件分岐

対象: `backend/src/kotorelay/context.py:96`。式: `not self.permission(doc.department_id, 'draft')`
...
対象: `backend/src/kotorelay/context.py:94`。式: `bool(rows) and rows[0].document_id == doc.id`
...
対象: `backend/src/kotorelay/context.py:98`。式: `digest(version.manifest.encode()) == version.manifest_hash`
...
対象: `backend/src/kotorelay/context.py:97`。式: `self.can_read(doc) and doc.latest_version_id == version.id`
...
対象: `backend/src/kotorelay/operations/reviews/functions.py:43`。式: `cached`
...
対象: `backend/src/kotorelay/operations/reviews/functions.py:61`。式: `data.decision == 'approved'`
...
対象: `backend/src/kotorelay/operations/reviews/functions.py:38`。式: `bool(rows)`
...
対象: `backend/src/kotorelay/operations/reviews/functions.py:45`。式: `submission.status == 'pending'`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F16-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F16-false | 不成立 | 'conflict' / 409 |


### F17 条件分岐

対象: `backend/src/kotorelay/operations/reviews/functions.py:47`。式: `version.created_by != ctx.user.id`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F17-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F17-false | 不成立 | 'self_approval' / 403 |


### F18 条件分岐

対象: `backend/src/kotorelay/operations/reviews/functions.py:48`。式: `submission.manifest_hash == data.manifest_hash == version.manifest_hash`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F18-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F18-false | 不成立 | 'conflict' / 409 |


### F19 条件分岐

対象: `backend/src/kotorelay/operations/reviews/functions.py:51`。式: `data.decision != 'rejected' or bool(data.reason.strip())`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F19-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F19-false | 不成立 | 'reason_required' / 422 |


### F20 条件分岐

対象: `backend/src/kotorelay/operations/reviews/functions.py:65`。式: `not previous or previous[0].number < version.number`
