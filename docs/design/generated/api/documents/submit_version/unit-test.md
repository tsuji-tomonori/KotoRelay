...
対象: `backend/src/kotorelay/operations/documents/submit_version/functions.py:62`。式: `ocr.confirmed and ocr.status == 'ready'`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F09-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F09-false | 不成立 | 'ocr_unconfirmed' / 409 |


### F10 条件分岐

対象: `backend/src/kotorelay/operations/documents/submit_version/functions.py:41`。式: `row.revision == data.revision`
...
対象: `backend/src/kotorelay/operations/documents/submit_version/router.py:33`。式: `cached`
