...
対象: `backend/src/kotorelay/operations/documents/functions.py:307`。式: `doc.revision == data.revision`
...
対象: `backend/src/kotorelay/operations/documents/functions.py:308`。式: `data.status != 'deleted' or bool(data.reason.strip())`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F08-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F08-false | 不成立 | 'reason_required' / 422 |


### F09 条件分岐

対象: `backend/src/kotorelay/operations/documents/functions.py:310`。式: `set(data.shared_departments) <= departments`

| 要素ID | 要素 | 期待観点 |
| --- | --- | --- |
| F09-true | 成立 | 成立側の実装を実行。正常／異常は上記式と処理に依存する。 |
| F09-false | 不成立 | 'forbidden' / 422 |
